...
import com.asianpaints.core.customer.facade.impl.DefaultAsianpaintsCustomerFacade;
...
public class AccountPageController extends AbstractSearchPageController
{
	...
	private static final String REDIRECT_TO_UPDATE_CONTACTNUMBER_PAGE = REDIRECT_PREFIX + "/my-account/update-contactNumber";
	...
	private static final String UPDATE_CONTACTNUMBER_CMS_PAGE = "update-contactNumber";
	...
	private DefaultAsianpaintsCustomerFacade defaultAsianpaintsCustomerFacade;
	...
	@Resource(name = "asianpaintsContactNumberValidator")
	private Validator asianpaintsContactNumberValidator;
	...
	/*
	 * Added For update Contact Number
	 */

	@RequestMapping(value = "/update-contactNumber", method = RequestMethod.GET)
	@RequireHardLogIn
	public String editContactNumber(final Model model) throws CMSItemNotFoundException
	{
		final CustomerData customerData = customerFacade.getCurrentCustomer();
		final UpdateContactNumberForm updateContactNumberForm = new UpdateContactNumberForm();

		updateContactNumberForm.setContactNumber(customerData.getContactNumber());

		model.addAttribute("updateContactNumberForm", updateContactNumberForm);

		storeCmsPageInModel(model, getContentPageForLabelOrId(UPDATE_CONTACTNUMBER_CMS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(UPDATE_CONTACTNUMBER_CMS_PAGE));
		model.addAttribute("breadcrumbs", accountBreadcrumbBuilder.getBreadcrumbs("text.account.profile"));
		model.addAttribute(ThirdPartyConstants.SeoRobots.META_ROBOTS, ThirdPartyConstants.SeoRobots.NOINDEX_NOFOLLOW);
		return getViewForPage(model);
	}
	...
	@RequestMapping(value = "/update-contactNumber", method = RequestMethod.POST)
	@RequireHardLogIn
	public String updateContactNumberForm(final UpdateContactNumberForm updateContactNumberForm,
			final BindingResult bindingResult, final Model model, final RedirectAttributes redirectAttributes)
			throws CMSItemNotFoundException
	{
		asianpaintsContactNumberValidator.validate(updateContactNumberForm, bindingResult);
		String returnAction = REDIRECT_TO_UPDATE_CONTACTNUMBER_PAGE;

		final CustomerData customerData = new CustomerData();
		customerData.setContactNumber(updateContactNumberForm.getContactNumber());

		if (!bindingResult.hasErrors()
				&& !updateContactNumberForm.getContactNumber().equals(updateContactNumberForm.getChkContactNumber()))
		{
			bindingResult.rejectValue("chkContactNumber", "validation.checkContactNumber.equals", new Object[] {},
					"validation.checkContactNumber.equals");
		}

		if (bindingResult.hasErrors())
		{
			returnAction = setErrorMessagesAndCMSPage(model, UPDATE_CONTACTNUMBER_CMS_PAGE);
		}
		else
		{
			try
			{
				defaultAsianpaintsCustomerFacade.updateContact(customerData);
				GlobalMessages.addFlashMessage(redirectAttributes, GlobalMessages.CONF_MESSAGES_HOLDER,
						"text.account.profile.confirmationUpdated", null);

				// Replace the spring security authentication with the new UID
				/*
				 * final String newUid = customerFacade.getCurrentCustomer().getUid().toLowerCase(); final Authentication
				 * oldAuthentication = SecurityContextHolder.getContext().getAuthentication(); final
				 * UsernamePasswordAuthenticationToken newAuthentication = new UsernamePasswordAuthenticationToken(newUid,
				 * null, oldAuthentication.getAuthorities()); newAuthentication.setDetails(oldAuthentication.getDetails());
				 * SecurityContextHolder.getContext().setAuthentication(newAuthentication);
				 */
			}
			catch (final DuplicateUidException e)
			{
				bindingResult.rejectValue("contactNumber", "profile.contactNumber.unique");
				returnAction = setErrorMessagesAndCMSPage(model, UPDATE_CONTACTNUMBER_CMS_PAGE);
			}
			catch (final PasswordMismatchException passwordMismatchException)
			{
